...
P_G = 0.85;            %gating size in percentage
...
nearestNeighborEstimates_ref = nearestNeighbourFilter(tracker_ref, initial_state, measdata, sensor_model, motion_model, meas_model);
nearestNeighborRMSE_ref = RMSE(nearestNeighborEstimates_ref,objectdata.X)

% Compare.
assert(abs(nearestNeighborRMSE-nearestNeighborRMSE_ref) < 1e-3, 'Root Mean Square Error: Expected %f, got %f.', nearestNeighborRMSE_ref, nearestNeighborRMSE);
assessVariableEqual('nearestNeighborEstimates', nearestNeighborEstimates_ref,'RelativeTolerance',0.03,'Feedback','Your Nearest Neighbour Filter implementation is incorrect, please modify your code and submit again.');