...
classdef GaussianDensity
    ...
    methods (Static)
        ...
        function predicted_likelihood = predictedLikelihood(state_pred,z,measmodel)
            ...
            [~,N] = size(z);
            predicted_likelihood = zeros(1,N);
            
            for n = 1:N
               predicted_likelihood(n) = log_mvnpdf(z(:,n), z_bar, S);
            end
        ...
        function state = momentMatching(w, states)
            ...
            for n = 1:N
                x_bar = x_bar + states(n).x .* w(n);
            end

            % covariance
            P_bar = zeros(size(states(1).P));
            for n = 1:N
                d = x_bar - states(n).x;
                P_bar = P_bar + (states(n).P + d * d').* w(n);
            end
